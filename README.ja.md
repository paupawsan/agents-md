...
1. **ファイルを取得** - GitHubからZIPとしてダウンロード（Git不要！）またはこのリポジトリをクローン
...
**💡 ヒント**: 
- **Gitがなくても大丈夫！** GitHubからリポジトリをZIPファイルとしてダウンロードできます - 「Code」→「Download ZIP」をクリックするだけです
- CursorまたはVS Codeを使用している場合、ワークスペースベースのセットアップ（オプションA）の方がはるかに簡単です - 各プロジェクトに`AGENTS.md`をコピーする必要がありません！
...
リポジトリファイルを取得するには、いくつかのオプションがあります：

#### オプションA: ZIPとしてダウンロード（技術者以外の方には最も簡単）

1. GitHubで、緑色の**「Code」**ボタンをクリック
2. **「Download ZIP」**をクリック
3. ZIPファイルをコンピューター上の場所に展開
4. 展開された`agents-md`フォルダに移動

**Gitは不要です！** コマンドラインツールに慣れていない場合、これが最も簡単な方法です。

#### オプションB: Gitでクローン（開発者向け）
...
**⚠️ 重要**: 言語を切り替えた後、スクリプトは設定された`{MEMORY_PATH}`を自動的に保持しようとします。ただし、スクリプトが設定されたパスを検出できない場合、新しい言語テンプレートでメモリパスを設定するために`setup_memory_dir.py`を再度実行する必要があります：

```bash
# 言語を切り替えた後、MEMORY_PATHを設定する必要がある場合：
python3 setup_memory_dir.py
```

スクリプトは`{MEMORY_PATH}`がまだ設定する必要がある場合に警告を表示します。
...
**⚠️ 重要**: 手動でコピーした後、メモリパスを**必ず**設定してください：
1. `setup_memory_dir.py`を実行して`{MEMORY_PATH}`を設定：
   ```bash
   python3 setup_memory_dir.py
   ```
   または、コピーした`AGENTS.md`の`{MEMORY_PATH}`プレースホルダーを完全なメモリルートパスに手動で置き換える（ステップ4を参照）
...
- **クリティカルシンキングシステム**: クリティカルシンキングフレームワークのドキュメントについては`_agents-md/critical-thinking/`を参照
  - **検証プロトコル**: `_agents-md/critical-thinking/verification-protocols.md` - グラウンドチェックと検証ワークフロー
  - **証拠に基づく推論**: `_agents-md/critical-thinking/evidence-based-reasoning.md` - クリティカルシンキングと異議申し立てプロトコル
  - **幻覚防止**: `_agents-md/critical-thinking/hallucination-prevention.md` - 誤情報を避けるためのガイドライン
